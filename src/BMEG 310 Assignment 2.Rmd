...
```{r}
glm.fit <- glm(as.factor(ovarian.dataset.train$diagnosis)~., data = ovarian.dataset.train[,3:32], 
               family = binomial)
...
```{r}
glm.probs <- predict(glm.fit, ovarian.dataset.test,
                     type = "response")
...
```{r}
ovarian.pca5 <- as.data.frame(ovarian.pca$x[,1:5])
ovarian.pca5$diagnosis <- as.factor(ovarian.dataset$diagnosis)

ovarian.dataset.train.pca5 <- ovarian.pca5[sample(nrow(ovarian.pca5))[1:(nrow(ovarian.pca5)/2)],]
ovarian.dataset.test.pca5 <- ovarian.pca5[sample(nrow(ovarian.pca5))[(nrow(ovarian.pca5)/2):(nrow(ovarian.pca5))],]

glm.fit.pc5 <- glm(as.factor(ovarian.dataset.train.pca5$diagnosis) ~., data = as.data.frame(ovarian.dataset.train.pca5), family = binomial)
...
table(glm.pred.pc5,ovarian.dataset.test.pca5$diagnosis)
mean(glm.pred.pc5 == ovarian.dataset.test.pca5$diagnosis)
```
...
### Q3.4
Classification gives better results because the accuracy of classification is higher. clustering by similarity may cause the conditions for grouping the obtained data do not satisfy our needs, while in classification we define the boundary conditions corresponding to specific categories and place the objects in classification, we define the boundary conditions for specific categories and place the objects into these categories, so that the results meet our needs better
